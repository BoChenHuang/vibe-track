...
本專案全程採用 AI 輔助開發，涵蓋以下三個層次：

- **Claude Code** — 互動式 CLI，用於架構設計討論、Spotify API 限制的排查與繞過方案、prompt 迭代，以及 NestJS module wiring、Guard / Interceptor 實作細節、Redis fail-open 錯誤處理等具體程式碼建議
- **Everything Claude Code (ECC)** — 專業化 sub-agent 套件，在程式碼審查（`code-reviewer`）、安全掃描（`security-reviewer`）、TypeScript 型別分析（`typescript-reviewer`）等任務上提供深度自動化輔助
- **OpenSpec** — 結構化變更追蹤工作流，每個功能異動都有對應的 `proposal → design → specs → tasks` 文件，確保需求、設計與實作三者一致，並在完成後歸檔至 `openspec/changes/archive/`
